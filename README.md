...
1. Использование SAGA паттерна для организация поездок. 
Например: Заказ поездки = цепочка: создание поездки -> назначение водителя -> расчёт стоимости -> оплата -> уведомление -> завершение.
2. Обработка graceful shutdown
3. Graceful degradation:
    - Геокодер не работает -> предложить указать точку на карте и использовать координаты
    - Не работает API внешнего сервиса оплаты. Сохраняем все платёжные данные в YDB. После, как сервис оплаты заработает, использовать отложенную обработку запросов.
...
4. Предусмотрен уровень гарантии Exactly once в kafka 
5. Каждый сервис пишет логи и статистику в Prometheus, а также визуализируем в Grafana
6. Кэшируем данные о геокодировани и постройке маршрутов в микровервисах
