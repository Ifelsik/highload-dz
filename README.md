...
Первое, что нужно, так это определить координаты точек по адресу - геокодирование.
Для этого можно развернуть сервер Nominatim. 
Он принимает данные OSM, помещает их в PostgreSQL и индексирует их. Дополнительно данные можно реплецировать
