...
| **passenger_ratings** | Оценки пассажиров от водителей           | `rating_id`: Уникальный идентификатор оценки (PK)<br> `passenger_id`: Внешний ключ, ссылающийся на таблицу **passengers**<br> `trip_id`: Внешний ключ, ссылающийся на таблицу **trips**<br> `rating`: Оценка пассажира (например, от 1 до 5) |
...
|-----------------------| -------- | ---- | ----------- |
| **users**             | 205 | 45 | Запрос на получение осуществляется каждый раз, как происходит вход пользователя в приложение. При записи учтено изменение данных из предположения, что каждый 10000-ый пользователь меняет данные |
| **passengers**        | 122 | 0.01 | Предположение, что пользовательские данные меняет кажый 10000-ый |
