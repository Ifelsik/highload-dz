...
| **Yandex Object Storage S3** | **Объектное хранилище** | Гибкость, масштабируемость, надёжность |
| **Prometheus + Grapfana** | **мониторинг** | Сбор метрик, отрисовка графиков, поддержка kubernetes |
| **GitLab** | **CI/CD** | Удобное и популярное решение, поддержка git |
...
* Физическая:
    - Дублирование питания ДЦ
    - Резерирование сетевых карт, провайдеров
    - Резервирование серверов
* Использование SAGA паттерна с оркестратором для организация поездок. Оркестратор на уровне
сервиса управления поездками.
Например: Заказ поездки = цепочка: создание поездки -> назначение водителя -> расчёт стоимости -> оплата -> уведомление -> завершение.
Используется дефолтный режим работы - at-least-once, то есть сообщение будет доставлено по крайней мере один раз и возможны дубли.
Для обеспечения выполнения только одной операции на уровне сервиса используются индемпотентые потребители, то есть например,
при получении последующих сообщений о необходимости проведения оплаты можно проверить, есть ли запись об оплате в БД с таким же
trip_id.    
Для этого следует настроить идемпотентность producer и его транзакционность (сообщений и оффсета).
* CQRS - раздельный синхронно-асинхронный пайплайн для чтения/записи:
    - Для чтения и записи в YDB рейтинга водителя и пассажиров
* Graceful shutdown - при остановке сервиса должны дообработаться запросы
* Graceful degradation:
    - Невозможность обновить данные в профиле -> отображаем старые данные с уведомлением, что изменения можно повторить позже
    - Сбои с микросервисом расчёта стоимости поездки (перегружен или ошибки внешних сервисов, например пробок).
    Вывести либо упрощённый расчёт за км и минуты, либо по таксометру. Лучше недополучить деньги, чем совсем их потерять.
    - Проблема с поиском ближайшего водителя (упал Redis). Можно предложить случайному водителю с указанием адреса посадки, если ему недалеко, то он, вероятно, согласится
    - Геокодер не работает -> предложить указать точку на карте и использовать координаты
    - Не работает API внешнего сервиса оплаты. Сохраняем все платёжные данные в YDB. После, как сервис оплаты заработает, использовать отложенную обработку запросов. Как альтернативу можно предложить оплатить наличными.
    - В случае выхода из строя Redis в микросервисе оценок, используем последнее вычисленное зачение из YDB. Все новые значения записываем в YDB. После, как работоспособность восстановится, отложенно пересчитать.
    - Упал микросервис чата. Отображаем в интерфейсе, что чаты времнно недоступны. Можно дополнительно вывести номер телефона водителя.
    - Не работает микросервис истории поездок. Некритично, можно просто отобразить, что указанный сервис временно недоступен.
    - API-шлюз прегружен. В этом случае можно ограничить запросы к некритичным сервисам, например истории поездок или чатам.
* Observability. 
    - Каждый сервис пишет логи
    - Статистика в Prometheus, визуализация в Grafana
    - Трассировка по request_id
    - Сохранение 10% логов в S3
    - Обработка инцидентов
    - При привышении критичных метрик настроены алерты.
* Кэшируем данные о геокодировани и постройке маршрутов, истории поездок, рейтинге водителя в микровервисах

 ## 10. Схема проекта

 ![сервисы](img/highload-services.png)

## 11. Список серверов

Расчёт аппартных ресурсов

| Технолония    | Характер сервиса | RPS  | RAM   |  
| - | - | - | - |
| FastHTTP (Go) |  сериализация JSON   | 5500 | 15 MB |
...
Nginx может поддерживать до 10000 одновременных HTTPS-соединений в секунду на 24 ядрах CPU [21]

### Расчёт базовой нагрузки

Нагрузка на ДЦ можно считать примерно равномерной согласно пункту 3.
Рассчитаем пиковую нагрузку с запасом на 1 вышедший из строя ДЦ.
Коэффициент выберем  
$K = \frac{L_{макс}}{L_{ср}} (1 + K_x + K_d)$,  
где:  
$L_{макс} = 4304$ - RPS на самый загруженный ДЦ,  
$L_{ср} = 3479$ - средний по ДЦ RPS,    
...
RPS = 152 (создание) + 152 (поход в маршрутизатор) + 152 (поход в калькулятор стоимости поездки) +
152 (поход в поисковик ближайшего водителя) + 152 (рейтинг) + 152 (поход в сервис оплаты) +
30 (получение данных о статусе поездок) = 942


| Сервис | Целевая пиковая нагрузка, RPS | Сеть, Гбит/с |
| - | - | - |
| Авторизация | K x 190 = 407 | 407 * (0,0048 + 0,0048) = 0.019 |
| Пользователи | K x 496 = 1062 | 540 Б * 1062 = 0.005 Гб/с |
| Геокодирование | K x 170 = 364 | 200 Б * 364 = 0.0006 Гб/с |
| Маршрутизация | K x 340 = 728 | 12 Б * 728 = 0.00007 Гб/с |
| Геолокация | K x 40940 = 87680 | 0.04 |
| Расчёт стоимости поездки | K x 340 = 728 | 25,3 КБ * 8 * 728 = 0.15 Гб/с |
| Управление поездками | K x 942 = 2017 | 483 * 8 * 2017 = 0.008 Гб/с |
| Чаты | K x 38 = 81 | 240 Б * 8 * 81 = 0.0001 Гб/с |
| Рейтинг | K x 274 = 587 | 92 Б * 8 * 587 = 0.0004 Гб/с |
| Оплата | K x 152 = 325 | 325 * 8 * 72 Б = 0.0002 Гб/с |
| История поездок | K x 3 = 7 | 0.00003 |
| API-gateway | 9253 | 0.278 |
| S3 | 577 фото + 170 gpx = 647 | 0.816 |
| nginx | 97553 | 1.134 |

Сделаем ряд доущений. Положим, что на поддержания 1 соединения по WS требуется порядка 70 КБ RAM [24]. Также положим, что на
обработку легкой бизнес-логики на 100 RPS требуется в 3 раза оперативной меньше памяти, чем при HTTP.

Рассчитаем аппаратные ресурсы на каждый сервис
| Сервис                  | Целевая пиковая нагрузка, RPS | Характер бизнес-логики | CPU, ядер            | RAM, ГБ            |
|-------------------------|-------------------------------|------------------------|----------------------|--------------------|
| Авторизация             | 407                           | лёгкая                 | 1                    | 5                  |
| Пользователи            | 1 062                         | средняя                | 11                   | 106                |
| Геокодирование          | 364                           | тяжёлая                | 37                   | 37                 |
| Маршрутизация           | 728                           | тяжёлая                | 73                   | 73                 |
| Геолокация              | 87 680                        | лёгкая                 | 88                   | 292               |
| Расчёт стоимости        | 728                           | средняя                | 8                    | 73                 |
| Управление поездками    | 2 017                         | средняя                | 21                   | 202                |
| Чаты                    | 81                            | лёгкая                 | 1                    | 1                  |
| Рейтинг                 | 587                           | средняя                | 6                    | 59                 |
| Оплата                  | 325                           | средняя                | 4                    | 33                 |
| История поездок         | 7                             | лёгкая                 | 1                    | 1                  |
| API-gateway             | 9 253                         | лёгкая                 | 10                   | 93                 |
| nginx (TLS-handshake)   | 97 553                        | TLS-handshake          | 24*8                 | 128                |
...
Рассчитаем аналогично для баз данных, учтём запас на расширение (40%) и архитектуру (mirror-3-dc в YDB и реплики Redis). Положим, что на YDB на 1 CPU будет 250 IOPS.
Аналогично для Redis будет примерно  1 CPU для 50k IOPS. При расчёте RAM учитаны размеры индексов, а также
использование mirror-3-dc.

| Сервис | Максимальный целевой IOPS | CPU | RAM, ГБ |
| - | - | - | - |
| YDB | 7614*| 31 |  1284 |
| Redis | 102441** | 14 | 152 |

\* Суммарно на все 3 ДЦ.  
\** Суммарно с репликами.

### Конфигурация серверов

Конфигурация собственных серверов подобрана согласно Broadberry [22]  
Для расчёта аренды в облаке использован Yandex Cloud [23]  

| Сервис | Конфигурация | Всего | Цена ед., $ | Суммарно, $ | Амортизация, $ | Аренда, $/мес |
| - | - | - | - | - | - | - |
| Kuber node | 16 core Xeon 4314/ 4x16 GB/ 1000GB NVMe M.2/ 2x25Gb/s | 31 | 2912 | 90 272 | 1 505 | 16 616 |
YDB | 8 core Xeon 3206R/ 2x64 GB/ 2x3.8 TB SSD SATA3/ 2x25Gb/s  |  6 | 3974 | 23 844 | 397 | 5 873 |
Redis | 6 core Xeon 3204/ 4x16 GB/ 1920 GB NVMe M.2/ 2x25Gb/s | 13 | 2407 | 31 291 | 522 | 4767 |
nginx | 10 core Xeon 4210/ 2x16 GB/ 512 GB NVMe M.2/ 4x25Gb/s | 13 | 2546 | 33 098 | 552 | 5723 |

Получаем, что стоимость покупки собственных серверов $178 505 и амортизация $2976 в месяц, аренда в месяц составит $32 979
Таким образом, собственные сервера окупятся уже через 6 месяцев.

Распределение собственных серверов по ДЦ
| Сервис | Москва | Санкт-Петербург | Самара | Краснодар | Новосибирск | Алмата |
| -      |    -   |          -      |   -    |     -     |      -      |    -   |
| Kuber nodes | 8 | 4  | 4 | 5 | 6 | 4 |
| YDB         | 2 | - | - | 2 | 2 | - |
| Redis       | 3 | 2 | 2 | 2 | 2 | 2 |
| nginx       | 3 | 2 | 2 | 2 | 2 | 2 | 
...
20. https://sharkbench.dev/web/go-fasthttp
21. https://blog.nginx.org/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers
22. https://www.broadberry.eu/rackmount-servers
23. https://yandex.cloud/ru/prices
24. https://www.reddit.com/r/reactjs/comments/18ogc0o/web_sockets_are_not_efficient_and_hard_to_scale